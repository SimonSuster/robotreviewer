...
def extract_txt_articles(articles):
    ...
    for article in articles:
        txt_articles.append(article.data["grobid"].get("title", "") + " " + article.data["grobid"].get("text", ""))
        abstract_articles.append(article.data["grobid"].get("abstract", ""))

    return txt_articles, abstract_articles


def to_output_format(pdffiles, txt_articles, abstract_articles, output):
    assert len(pdffiles) == len(txt_articles) == len(abstract_articles)

    for f, t, a in zip(pdffiles, txt_articles, abstract_articles):
        doi = os.path.splitext(os.path.basename(f))[0].replace("_", "/")
        output[doi] = {"text": t, "abstract": a}
...
def main():
    ...
    for n, pdffiles in enumerate(pdffiles_chunked):
        ...
        output = to_output_format(pdffiles, txt_articles, abstract_articles, output)
